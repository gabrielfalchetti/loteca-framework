...
Saída: data/out/<RODADA>/odds_theoddsapi.csv  (colunas: home,away,book,k1,kx,k2,total_line,over,under,ts)

Melhorias desta versão:
- Detecção de esportes BR por `title` (contém "Brazil" ou "Copa do Brasil") **e** por `key` (fallbacks).
- Fallback explícito para chaves conhecidas: serie_a, serie_b, serie_c, serie_d, copa do brasil.
- Logs de debug (`--debug`) indicando: esportes detectados, eventos por esporte e quantos tiveram odds.
- Saída sempre com colunas fixas, mesmo se a API retornar vazio (pipeline não quebra).

Requisitos:
- ENV: THEODDSAPI_KEY
- Input: data/in/<RODADA>/matches_source.csv com colunas mín.: home, away [,date]
Uso:
...
# Fallback explícito de chaves de esporte (conhecidas na TheOddsAPI, podem variar ao longo do tempo)
FALLBACK_BR_SPORT_KEYS = [
    # variações comuns que já vi para Brasil:
    "soccer_brazil_serie_a",
    "soccer_brazil_serie_b",
    "soccer_brazil_serie_c",
    "soccer_brazil_serie_d",
    "soccer_brazil_cup",
    # alguns provedores usam "soccer_brazil_campeonato":
...
def _list_brazil_sports(key: str, debug=False) -> List[str]:
    """Tenta detectar chaves de esportes do Brasil via /sports."""
    ...
    for it in js or []:
        ...
        skey  = it.get("key") or ""
        # Critérios:
        # 1) título menciona Brazil ou Copa do Brasil
        # 2) a key contém 'brazil' (mais robusto que title)
    ...
    if debug:
        print(f"[theoddsapi] sports listados pela API (BR): {sports_keys}")

    # Fallback: se nada detectado, injeta chaves conhecidas
    if not sports_keys:
        sports_keys = FALLBACK_BR_SPORT_KEYS.copy()
        if debug:
            print(f"[theoddsapi] fallback de chaves BR aplicado: {sports_keys}")
    return sports_keys

def _pull_odds_for_sport(key: str, sport_key: str, regions: str, debug=False) -> List[Dict[str, Any]]:
    """
    Puxa odds H2H (pre-match) para um sport_key (ex.: soccer_brazil_serie_a).
    Retorna lista de dicts com home, away, k1, kx, k2.
    """
    params = {
        "apiKey": key,
        "regions": regions,     # exemplo: "br,eu,uk,us"
        "markets": "h2h",
        "oddsFormat": "decimal"
    }
    ...
    for ev in js or []:
        ...
        # Agregação por média entre bookmakers (H2H)
...
def main():
    ...
    ap.add_argument("--regions", default="br,eu,uk,us", help='ex.: "br,eu,uk,us"')
    ...
    # Lê sua lista de jogos e prepara normalização para match futuro
    ...
    # 1) Detecta esportes BR
    sports = _list_brazil_sports(key, debug=args.debug)
    if args.debug and sports:
        print(f"[theoddsapi] BR sports a consultar: {sports}")

    # 2) Coleta odds por esporte
    ...
    # 3) Monta DF com colunas garantidas
    rows = []
    for it in pulled:
    ...
    # 4) Faz o match com seus jogos (igualdade -> fuzzy 0.90)
    out_rows = []
    if not odds.empty:
        for _, m in matches.iterrows():
            ...
            if hit.empty:
                # fuzzy simples com média de similaridade
                best = None
                best_sc = 0.0
        ...
        if args.debug:
            print("[theoddsapi] Nenhum evento BR com odds H2H retornado pela API neste momento.")

    # 5) Salva CSV (colunas fixas mesmo vazio)
    df = pd.DataFrame(out_rows, columns=OUT_COLS)
    df.to_csv(out_path, index=False)

    if args.debug:
        print(f"[theoddsapi] arquivo salvo: {out_path} ({len(df)} linhas)")

    # Log final padrão do seu pipeline
