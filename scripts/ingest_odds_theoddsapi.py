...
# Lê matches_source.csv, coleta odds no TheOddsAPI resolvendo sport_keys dinamicamente
# e grava data/out/<rodada>/odds_theoddsapi.csv (+ unmatched para depuração).

from __future__ import annotations
import argparse, csv, os, sys, json
from pathlib import Path
from typing import Dict, Any, List, Tuple
from utils.oddsapi import resolve_brazil_soccer_sport_keys, fetch_odds_for_sport, _norm, OddsApiError
...
def flatten_odds(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    TheOddsAPI → flatten para: provider_event_home, provider_event_away, bookmaker, market, selection, price
    """
    flat = []
    for ev in events:
...
def match_provider_events(matches: List[Dict[str,str]], prov_rows: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Casa prov_rows (TheOddsAPI) com matches pelo par (home,away) normalizado/fuzzy.
    Retorna (casados, nao_casados)
    """
    out, unmatched = [], []
    # índice por par canônico
    prov_index: Dict[Tuple[str,str], List[Dict[str, Any]]] = {}
    for r in prov_rows:
        h = canonical(r["prov_home"])
        a = canonical(r["prov_away"])
    ...
    for m in matches:
        ...
        if not rows:
            ...
            cand_home = [k[0] for k in cand_keys]
            cand_away = [k[1] for k in cand_keys]
            h_best = fuzzy_match(m["home"], cand_home)
            a_best = fuzzy_match(m["away"], cand_away)
        ...
        if rows:
            ...
            unmatched.append({
                "match_id": m["match_id"], "home": m["home"], "away": m["away"], "motivo": "no_match_theoddsapi"
            })
...
def main():
    ...
    rows = read_matches(ms_path)

    print("[theoddsapi] Resolvendos sport_keys do Brasil…")
    keys = resolve_brazil_soccer_sport_keys()
    if args.debug:
        print(f"[theoddsapi] sport_keys candidatos: {keys}")
    ...
    if unmatched:
        ...
        print(f"[theoddsapi] AVISO: {len(unmatched)} jogos sem casamento. Veja {um_csv}")
