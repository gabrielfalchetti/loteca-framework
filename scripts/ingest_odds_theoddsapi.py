...
def match_team(api_name: str, source_teams: list, threshold: float = 70) -> str:
    ...
    for source_team in source_teams:
        ...
        if fuzz.ratio(api_norm, source_norm) > threshold:
            return source_team
    return None
...
def fetch_stats(rodada: str, source_csv: str, api_key: str, api_key_theodds: str, regions: str) -> pd.DataFrame:
    ...
    for game in fixtures:
        home_team = normalize_team_name(game["home_team"] if is_theoddsapi else game["teams"]["home"]["name"])
        away_team = normalize_team_name(game["away_team"] if is_theoddsapi else game["teams"]["away"]["name"])
        fixture_id = game["id"] if is_theoddsapi else game["fixture"]["id"]
    ...
    for _, row in matches_df.iterrows():
        ...
        if fixtures_data and fixtures_data.get("response"):  # API-Football
            try:
        ...
        odds_data = None
        for game in fixtures_data if is_theoddsapi else []:
