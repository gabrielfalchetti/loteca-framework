...
def generate_consensus(theodds_file, apifootball_file, output_file):
    ...
    try:
        ...
        _log(f"Carregado {apifootball_file} com {len(apifootball_df)} linhas")
    except Exception as e:
    ...
    if 'team_home' in apifootball_df.columns:
        apifootball_df = apifootball_df.rename(columns={'team_home': 'home_team', 'team_away': 'away_team'})

    # Merge dos DataFrames usando 'home_team' e 'away_team'
    try:
        consensus = theodds_df.merge(apifootball_df, on=['home_team', 'away_team'], how='outer', suffixes=('_theodds', '_api'))
    ...
    consensus['home_odds'] = consensus[['home_odds_theodds', 'home_odds_api']].mean(axis=1).fillna(2.0)
    consensus['draw_odds'] = consensus[['draw_odds_theodds', 'draw_odds_api']].mean(axis=1).fillna(3.0)
    consensus['away_odds'] = consensus[['away_odds_theodds', 'away_odds_api']].mean(axis=1).fillna(2.5)
