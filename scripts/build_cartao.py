...
Execução:
  python -m scripts.build_cartao --rodada "<OUT_DIR opcional>"
"""
...
def main():
    # aceita OUT_DIR direto ou monta com RODADA_ID
    ...
    if out_dir_env:
        ...
        out_dir = Path(f"data/out/{rid}")

    out_dir.mkdir(parents=True, exist_ok=True)

    wl_path = Path("data/in/matches_whitelist.csv")
    if not wl_path.exists():
        fail("[cartao] matches_whitelist.csv ausente. Rode 'python -m scripts.match_whitelist' antes.")
    ...
    for i, r in enumerate(wl.itertuples(index=False), start=1):
        ...
        if not kelly.empty and {"team_home", "team_away", "pick", "stake"}.issubset(kelly.columns):
            ...
            if not ksel.empty:
                pick = {"HOME": "1", "DRAW": "X", "AWAY": "2"}.get(str(ksel.iloc[0]["pick"]).upper(), "?")
        ...
        if pick == "?" and not blend.empty and {"team_home", "team_away", "p_home", "p_draw", "p_away"}.issubset(blend.columns):
            ...
            if not b.empty:
                ph = float(b.iloc[0]["p_home"]); pd_ = float(b.iloc[0]["p_draw"]); pa = float(b.iloc[0]["p_away"])
        ...
        if pick == "?" and not cons.empty and {"team_home", "team_away", "odds_home", "odds_draw", "odds_away"}.issubset(cons.columns):
            ...
            if not c.empty:
                oh = float(c.iloc[0]["odds_home"]); od = float(c.iloc[0]["odds_draw"]); oa = float(c.iloc[0]["odds_away"])
